...
export class KiloProvider implements AIProvider {
  readonly id = 'kilo';
  readonly name = 'Kilo (Developer Specialist)';
  ...
  private readonly baseUrl = process.env.KILO_BASE_URL || 'https://api.kilo.ai/v1';
  ...
  isConfigured(): boolean {
    return Boolean(process.env.KILO_API_KEY && process.env.KILO_API_KEY.trim());
  }
  ...
  async generate(request: GenerateRequest): Promise<GenerateResponse> {
    ...
    if (!key) throw new Error('KILO_API_KEY is not configured');
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Kilo returned ${res.status}: ${text.slice(0, 150)}`);
      }
...
export const kiloProvider = new KiloProvider();
