...
interface StudioControlsProps {
  ...
  lastDiagnostic?: ExecutionDiagnostic | null;
...
export const StudioControls: React.FC<StudioControlsProps> = ({
  ...
  const [providers, setProviders] = useState<ProviderSafeStatus[]>([]);
  ...
  useEffect(() => {
    if (activeTab === 'brain' || isSettingsOpen) {
      fetchProviders();
    }
  }, [activeTab, isSettingsOpen, fetchProviders]);
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-sm text-neutral-300">
              ...
              {activeTab === 'brain' && (
                <div className="space-y-6">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-xs font-semibold text-neutral-200 uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        AI Brain & Provider Orchestration
                      </label>
                      ...
                      {[
                        {
                          ...
                          desc: 'Gemini Primary Brain with intelligent intent routing & automatic multi-provider fallback.',
                          details: 'Gemini 2.5 Flash → OpenAI → Groq → OpenRouter',
                        },
                        {
                          id: 'gemini',
                          ...
                          desc: 'Direct execution on Google Gemini 2.5 Flash / 2.5 Pro with deep multimodal understanding.',
                          details: 'Model: gemini-2.5-flash / gemini-2.5-pro',
                        },
                        {
                          id: 'openai',
                          title: 'OpenAI',
                          ...
                          desc: 'High emotional nuance, conversational agility, and structured animation orchestration.',
                          details: 'Model: gpt-4o / gpt-4o-mini',
                        },
                        {
                          id: 'groq',
                          title: 'Groq LPU',
                          ...
                          desc: 'Sub-200ms ultra-low latency inference using Llama 3.3 70B Versatile on custom LPUs.',
                        ...
                        },
                        {
                          ...
                          desc: 'Unified multi-provider fallback gateway accessing top global models seamlessly.',
                          details: 'Model: auto-routing fallback',
                        },
                        {
                          id: 'huggingface',
                          title: 'Hugging Face',
                          ...
                          desc: 'Serverless inference on open-source weights (Qwen, Llama, Mistral).',
                          details: 'Model: Qwen2.5-72B-Instruct',
                        },
                        {
                          id: 'opencode',
                          title: 'OpenCode',
                          ...
                          desc: 'Dedicated coding and technical reasoning specialist for developer inquiries.',
                          details: 'Model: deepseek-coder / qwen-coder',
                        },
                        {
                          id: 'kilo',
                          title: 'Kilo Gateway',
                          ...
                          desc: 'High-speed compatible assistant gateway for complementary reasoning.',
                          details: 'Model: versatile-assistant',
                        },
                      ].map((item) => {
                        const isSelected = (config.aiBrain || 'auto') === item.id;
                        return (
                          <button
                            key={item.id}
                            onClick={() => onUpdateConfig({ aiBrain: item.id as AIBrainMode })}
                          ...
                          >
                            ...
                            <p className="text-[11px] text-neutral-300 leading-relaxed mb-1.5 line-clamp-2">
                              {item.desc}
                            </p>
                  ...
                  <div className="p-3.5 rounded-2xl bg-neutral-950/50 border border-white/10 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5 text-xs font-semibold text-white">
                        ...
                        Live Provider Health & Latency
                      </div>
                      <button
                        onClick={fetchProviders}
                        disabled={isLoadingProviders}
                        className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-white/5 hover:bg-white/10 text-[10px] text-neutral-300 border border-white/10 transition"
                      >
                        <RefreshCw className={`w-3 h-3 ${isLoadingProviders ? 'animate-spin' : ''}`} />
                        Refresh
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {providers.map((p) => {
                        const statusColors: Record<string, { bg: string; text: string; dot: string }> = {
                          healthy: { bg: 'bg-emerald-500/10 border-emerald-500/30', text: 'text-emerald-300', dot: 'bg-emerald-400' },
                          degraded: { bg: 'bg-amber-500/10 border-amber-500/30', text: 'text-amber-300', dot: 'bg-amber-400' },
                          cooldown: { bg: 'bg-blue-500/10 border-blue-500/30', text: 'text-blue-300', dot: 'bg-blue-400' },
                          offline: { bg: 'bg-red-500/10 border-red-500/30', text: 'text-red-300', dot: 'bg-red-400' },
                          unconfigured: { bg: 'bg-neutral-900 border-white/5', text: 'text-neutral-500', dot: 'bg-neutral-600' },
                        };
                        const styling = statusColors[p.status] || statusColors.unconfigured;

                        return (
                          <div
                            key={p.id}
                            className={`p-2 rounded-xl border text-left flex flex-col justify-between ${styling.bg}`}
                          >
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-[11px] font-semibold text-neutral-200 truncate">{p.name}</span>
                              <span className={`w-1.5 h-1.5 rounded-full ${styling.dot} ${p.status === 'healthy' ? 'animate-pulse' : ''}`} />
                            </div>
                            <div className="flex items-center justify-between text-[9px] font-mono text-neutral-400">
                              <span className={styling.text}>
                                {p.status === 'healthy' ? 'Online' : p.status === 'unconfigured' ? 'Unconfigured' : p.status}
                              </span>
                              {typeof p.avgLatencyMs === 'number' && p.avgLatencyMs > 0 && (
                  ...
                  {lastDiagnostic && (
                    <div className="p-3.5 rounded-2xl bg-neutral-950/60 border border-purple-500/30 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-[9px] font-mono px-2 py-0.5 rounded-full ${
                          lastDiagnostic.fallbackUsed
                            ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30'
                            : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'
                        }`}>
                          {lastDiagnostic.fallbackUsed ? 'Fallback Invoked' : 'Primary Success'}
                        </span>
                      </div>

                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[10px] font-mono bg-neutral-900/80 p-2.5 rounded-xl border border-white/5">
                        <div>
                          <span className="text-neutral-500 block">Answered By:</span>
                          <span className="text-white font-semibold uppercase">{lastDiagnostic.selectedProvider}</span>
                        </div>
                        <div>
                          <span className="text-neutral-500 block">Model:</span>
                          <span className="text-purple-300 truncate block">{lastDiagnostic.selectedModel}</span>
                        </div>
                        <div>
                          <span className="text-neutral-500 block">Latency:</span>
                          <span className="text-emerald-400 font-semibold">{lastDiagnostic.latencyMs}ms</span>
                        </div>
                        <div>
                          <span className="text-neutral-500 block">Task Route:</span>
                          <span className="text-neutral-300">{lastDiagnostic.taskType}</span>
                        </div>
                      </div>

                      <div className="text-[10px] text-neutral-400 font-mono leading-relaxed pt-1">
                        <span className="text-neutral-500">Routing Reason: </span>
                        {lastDiagnostic.selectionReason}
                      </div>

                      {Array.isArray(lastDiagnostic.attemptedProviders) && lastDiagnostic.attemptedProviders.length > 1 && (
                        <div className="text-[10px] text-neutral-400 font-mono flex items-center gap-1 pt-0.5">
                          <span className="text-neutral-500">Attempted Chain: </span>
                          <span className="text-neutral-300">{lastDiagnostic.attemptedProviders.join(' → ')}</span>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="p-3.5 rounded-2xl bg-neutral-950/50 border border-white/10 space-y-2">
                    <div className="text-xs font-semibold text-white flex items-center gap-1.5">
                      ...
                      Multi-Provider Orchestrator Pipeline
                    </div>
                    <div className="text-[11px] text-neutral-400 space-y-1 font-mono leading-relaxed bg-neutral-900/60 p-2.5 rounded-xl border border-white/5">
                      <div className="text-purple-300 font-semibold">User Query / Speech</div>
                      <div className="text-neutral-500">  ↓ Orchestrator Router (Intent, Task Type, Locale)</div>
                      <div className="text-neutral-300">  ├── 1. Primary Brain: Gemini 2.5 Flash / Pro (Default)</div>
                      <div className="text-neutral-300">  ├── 2. Coding Specialist: OpenCode / Groq / OpenAI</div>
                      <div className="text-neutral-300">  ├── 3. Fallback Chain: Gemini → OpenAI → Groq → OpenRouter → HF</div>
                      <div className="text-neutral-300">  └── 4. Output Schema: &#123; message, emotion, animation, language &#125; → Voice/VRM/UI</div>
                    </div>
                  </div>
