...
import { Emotion, CharacterAnimation, LightingPreset, CameraPreset, BackgroundPreset, VRMModelMeta, ColumbinaAction, ColumbinaFacialExpression } from '../types';
import { VRMActionController } from '../modules/vrmActionController';
import { VRMExpressionController } from '../modules/vrmExpressionController';
import {
  VRMBodyExpressionController,
  HandFingerSet,
  ActiveGestureState,
} from '../modules/vrmBodyExpressionController';
...
export const VRMViewer: React.FC<VRMViewerProps> = ({
  ...
  const facialExpressionRef = useRef<ColumbinaFacialExpression | string | undefined>(facialExpression);
  facialExpressionRef.current = facialExpression;

  const actionRef = useRef<ColumbinaAction | string | undefined>(action);
  ...
  const spokenTextRef = useRef<string>(spokenText);
  spokenTextRef.current = spokenText;

  const userEmotionRef = useRef<string>(userEmotion);
  userEmotionRef.current = userEmotion;
  ...
  const speechCueCheckedRef = useRef<boolean>(false);

  // Bone smooth rotation memory map
  const boneRotationCacheRef = useRef<Map<string, [number, number, number]>>(new Map());
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (vrm && characterLoadedRef.current) {
        ...
        // =========================================================================
        // NATURAL IDLE-FIRST KINEMATIC & EXPRESSION ENGINE
        // Dominant State: 80-90% Idle with hands down beside thighs/hips
        // =========================================================================
        const damping = Math.min(1.0, delta * 10.0); // Smooth fluid interpolation

        // Check for new intentional action request from AI
        const currentAction = actionRef.current;
        if (
          currentAction &&
          currentAction !== 'idle' &&
          currentAction !== lastProcessedActionRef.current
        ) {
          lastProcessedActionRef.current = currentAction;
          // Only trigger if no gesture is currently active and cooldown has elapsed
          if (!activeGestureRef.current && elapsedTime >= gestureCooldownUntilRef.current) {
            const normalized = VRMActionController.normalizeAction(currentAction);
            const handChoice: 'left' | 'right' = Math.random() > 0.35 ? 'right' : 'left';
            if (
              normalized === 'touch_chest' ||
              normalized === 'comforting_gesture' ||
              normalized === 'open_palm_gesture' ||
              normalized === 'point' ||
              normalized === 'thoughtful_pose' ||
              normalized === 'small_wave' ||
              normalized === 'greeting' ||
              normalized === 'small_head_tilt' ||
              normalized === 'nod' ||
              normalized === 'small_shoulder_movement' ||
              normalized === 'surprised_reaction'
            ) {
              activeGestureRef.current = {
                action: normalized,
                startTime: elapsedTime,
                duration: normalized === 'small_wave' ? 2.0 : 2.2,
                hand: handChoice,
                intensity: 0.5,
              };
            }
        ...
        // Check for subtle speech cue emphasis (only when starting to speak a new sentence)
        const currentSpokenText = spokenTextRef.current;
        if (isSpeakingRef.current && currentSpokenText && currentSpokenText !== lastSpokenTextRef.current) {
          lastSpokenTextRef.current = currentSpokenText;
          speechCueCheckedRef.current = false;
        }

        if (
          isSpeakingRef.current &&
          !speechCueCheckedRef.current &&
          !activeGestureRef.current &&
          elapsedTime >= gestureCooldownUntilRef.current
        ) {
          speechCueCheckedRef.current = true;
          const cue = VRMBodyExpressionController.extractSubtleSpeechCue(spokenTextRef.current, 3.0);
          if (cue) {
            activeGestureRef.current = {
              action: cue.gesture,
              startTime: elapsedTime + cue.timeOffset,
              duration: 2.0,
              hand: cue.hand,
              intensity: 0.45,
            };
        ...
        // Clean up expired gesture and enforce natural return-to-idle cooldown (8-12s)
        if (activeGestureRef.current) {
          const gestureElapsed = elapsedTime - activeGestureRef.current.startTime;
          if (gestureElapsed >= activeGestureRef.current.duration) {
            activeGestureRef.current = null;
            gestureCooldownUntilRef.current = elapsedTime + (Math.random() * 4.0 + 8.0);
          }
        }

        const bodyPose = VRMBodyExpressionController.computePose({
          emotion: currentEmotionRef.current,
          facialExpression: facialExpressionRef.current,
          action: actionRef.current,
          isSpeaking: isSpeakingRef.current,
          isListening: isListeningRef.current,
          userEmotion: userEmotionRef.current,
          elapsedTime,
          intensity: emotionIntensityRef.current,
          activeGesture: activeGestureRef.current,
        });

        // Helper to smoothly apply bone rotations
        const applySmoothBone = (boneName: BoneName, target?: [number, number, number]) => {
          if (!target) return;
          const boneNode = vrm.humanoid?.getNormalizedBoneNode(boneName);
          if (!boneNode) return;

          const cacheKey = boneName;
          const cur = boneRotationCacheRef.current.get(cacheKey) || [
            boneNode.rotation.x,
            boneNode.rotation.y,
            boneNode.rotation.z,
          ];
          ...
          const next: [number, number, number] = [
            THREE.MathUtils.lerp(cur[0], target[0], damping),
            THREE.MathUtils.lerp(cur[1], target[1], damping),
            THREE.MathUtils.lerp(cur[2], target[2], damping),
          ];

          boneRotationCacheRef.current.set(cacheKey, next);
          boneNode.rotation.set(next[0], next[1], next[2]);
        };

        const capitalize = (s: string) => s.charAt(0).toUpperCase() + s.slice(1);

        const applyFingers = (side: 'left' | 'right', fingers?: HandFingerSet) => {
          if (!fingers || !vrm.humanoid) return;
          const prefix = side;
          const fNames = ['thumb', 'index', 'middle', 'ring', 'little'] as const;

          for (const f of fNames) {
            const fData = fingers[f];
            if (!fData) continue;

            if (fData.metacarpal) {
              const bName = `${prefix}${capitalize(f)}Metacarpal` as any;
              const node = vrm.humanoid.getNormalizedBoneNode(bName);
              if (node) {
                const cur = boneRotationCacheRef.current.get(bName) || [node.rotation.x, node.rotation.y, node.rotation.z];
                const nxt: [number, number, number] = [
                  THREE.MathUtils.lerp(cur[0], fData.metacarpal[0], damping),
                  THREE.MathUtils.lerp(cur[1], fData.metacarpal[1], damping),
                  THREE.MathUtils.lerp(cur[2], fData.metacarpal[2], damping),
                ];
                boneRotationCacheRef.current.set(bName, nxt);
                node.rotation.set(nxt[0], nxt[1], nxt[2]);
              }
            }
            if (fData.proximal) {
              const bName = `${prefix}${capitalize(f)}Proximal` as any;
              const node = vrm.humanoid.getNormalizedBoneNode(bName);
              if (node) {
                const cur = boneRotationCacheRef.current.get(bName) || [node.rotation.x, node.rotation.y, node.rotation.z];
                const nxt: [number, number, number] = [
                  THREE.MathUtils.lerp(cur[0], fData.proximal[0], damping),
                  THREE.MathUtils.lerp(cur[1], fData.proximal[1], damping),
                  THREE.MathUtils.lerp(cur[2], fData.proximal[2], damping),
                ];
                boneRotationCacheRef.current.set(bName, nxt);
                node.rotation.set(nxt[0], nxt[1], nxt[2]);
              }
            }
            if (fData.intermediate) {
              const bName = `${prefix}${capitalize(f)}Intermediate` as any;
              const node = vrm.humanoid.getNormalizedBoneNode(bName);
              if (node) {
                const cur = boneRotationCacheRef.current.get(bName) || [node.rotation.x, node.rotation.y, node.rotation.z];
                const nxt: [number, number, number] = [
                  THREE.MathUtils.lerp(cur[0], fData.intermediate[0], damping),
                  THREE.MathUtils.lerp(cur[1], fData.intermediate[1], damping),
                  THREE.MathUtils.lerp(cur[2], fData.intermediate[2], damping),
                ];
                boneRotationCacheRef.current.set(bName, nxt);
                node.rotation.set(nxt[0], nxt[1], nxt[2]);
              }
            ...
            if (fData.distal) {
              const bName = `${prefix}${capitalize(f)}Distal` as any;
              const node = vrm.humanoid.getNormalizedBoneNode(bName);
              if (node) {
                const cur = boneRotationCacheRef.current.get(bName) || [node.rotation.x, node.rotation.y, node.rotation.z];
                const nxt: [number, number, number] = [
                  THREE.MathUtils.lerp(cur[0], fData.distal[0], damping),
                  THREE.MathUtils.lerp(cur[1], fData.distal[1], damping),
                  THREE.MathUtils.lerp(cur[2], fData.distal[2], damping),
                ];
                boneRotationCacheRef.current.set(bName, nxt);
                node.rotation.set(nxt[0], nxt[1], nxt[2]);
              }
        ...
        // Apply Expressive & Living Body Bones (Hands, Arms, Shoulders, Spine, Hips, Neck, Head)
        applySmoothBone('head', bodyPose.head);
        applySmoothBone('neck', bodyPose.neck);
        applySmoothBone('spine', bodyPose.spine);
        applySmoothBone('chest', bodyPose.chest);
        applySmoothBone('leftShoulder', bodyPose.leftShoulder);
        applySmoothBone('rightShoulder', bodyPose.rightShoulder);
        applySmoothBone('leftUpperArm', bodyPose.leftUpperArm);
        applySmoothBone('leftLowerArm', bodyPose.leftLowerArm);
        applySmoothBone('leftHand', bodyPose.leftHand);
        applySmoothBone('rightUpperArm', bodyPose.rightUpperArm);
        applySmoothBone('rightLowerArm', bodyPose.rightLowerArm);
        applySmoothBone('rightHand', bodyPose.rightHand);

        // Core micro-balance & weight shift
        applySmoothBone('hips', bodyPose.hips || [0, 0, 0]);
        applySmoothBone('leftUpperLeg', bodyPose.leftUpperLeg || [0, 0, 0]);
        applySmoothBone('leftLowerLeg', [0, 0, 0]);
        applySmoothBone('leftFoot', [0, 0, 0]);
        applySmoothBone('rightUpperLeg', bodyPose.rightUpperLeg || [0, 0, 0]);
        applySmoothBone('rightLowerLeg', [0, 0, 0]);
        applySmoothBone('rightFoot', [0, 0, 0]);

        // Apply Left and Right Finger articulation
        applyFingers('left', bodyPose.leftFingers);
        applyFingers('right', bodyPose.rightFingers);
        ...
        if (isSpeakingRef.current && analyser) {
          ...
          if (currentRms > silenceThreshold) {
            ...
            const mouthScale = bodyPose.mouthScale || 1.0;
            const mouthOpen = THREE.MathUtils.clamp(((currentRms - silenceThreshold) / 0.12) * mouthScale, 0, 1.0);
        ...
        // 8. Expression Morphs (Powered by VRMExpressionController with comprehensive emotion & facial mapping)
        const emotion = currentEmotionRef.current;
        const facialExp = facialExpressionRef.current;
        const intensity = emotionIntensityRef.current;

        const controllerWeights = VRMExpressionController.computeExpressionWeights(
          emotion as any,
          facialExp as any,
          intensity
        );

        const finalBlink = Math.max(
          currentBlinkWeight,
          controllerWeights.blink || 0,
          emotion === 'sleepy' ? 0.45 : 0
        );

        const targetWeights: Record<string, number> = {
          happy: Math.min(1.0, (controllerWeights.happy || 0) + (bodyPose.mouthEmotionMod?.happyAdd || 0)),
          angry: controllerWeights.angry || 0,
          sad: Math.min(1.0, (controllerWeights.sad || 0) + (bodyPose.mouthEmotionMod?.sadAdd || 0)),
          relaxed: controllerWeights.relaxed || 0,
          surprised: Math.min(1.0, (controllerWeights.surprised || 0) + (bodyPose.mouthEmotionMod?.surprisedAdd || 0)),
          neutral: controllerWeights.neutral || 0,
          blink: finalBlink,
          blinkLeft: controllerWeights.blinkLeft || (emotion === 'wink' ? 1.0 : 0),
          blinkRight: controllerWeights.blinkRight || 0.0,
          aa: vowelA,
          ih: vowelI,
          ou: vowelU,
          ee: vowelE,
